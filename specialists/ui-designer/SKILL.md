...
- Do NOT Read entire reference files. Use Grep to search `~/.claude/skills/gteam/specialists/ui-designer/references/` for specific keywords relevant to the task (e.g. contrast, spacing, component names, WCAG criteria).
- Check `~/.claude/skills/gteam/specialists/ui-designer/results/` — if result entries exist, Grep them for tested patterns.
