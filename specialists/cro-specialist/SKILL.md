...
|---|---|---|
| Audit a landing page | `tasks/landing-page-audit.md` | Single page with traffic but low conversion |
| Analyse a funnel | `tasks/funnel-analysis.md` | Multi-step drop-off diagnosis |
| Design an A/B test | `tasks/ab-test-design.md` | Planning a specific experiment |
| Interpret heatmaps | `tasks/heatmap-interpretation.md` | Have Hotjar/Clarity/FullStory data to turn into insights |
| Build a CRO roadmap | `tasks/cro-roadmap.md` | Prioritising a backlog of test ideas over a quarter |

**Routing rules:**
1. Single page in scope → `landing-page-audit.md`.
2. Multi-step journey → `funnel-analysis.md`, then `ab-test-design.md` for the biggest leak.
3. Existing recordings/heatmaps to interpret → `heatmap-interpretation.md` first, then A/B design.
4. Quarterly planning / backlog grooming → `cro-roadmap.md`.

**Load task:** Read the task file, then execute its workflow.

## Reference Materials

- `~/dev/1_myprojects/gteam/specialists/cro-specialist/references/conversion-benchmarks.md` — industry conversion rate benchmarks by page type and vertical
- `~/dev/1_myprojects/gteam/specialists/cro-specialist/references/cro-tools.md` — testing platforms, heatmap tools, analytics integrations

**Shared psychology reference:** `~/dev/1_myprojects/gteam/references/marketing-psychology.md` — Quick-Reference table maps conversion challenges to psychological levers.

**Search discipline:**
- Do NOT Read entire reference files. Use Grep to locate the specific benchmark, lever, or pattern relevant to the task.
- Check `~/dev/1_myprojects/gteam/specialists/cro-specialist/results/` — if result entries exist, Grep them. Prefer `[TESTED]` over `[HYPOTHESIS]`.

## Notes

- Never recommend a change without a hypothesis. Every change has a reason and a measurable outcome.
- Most tests fail — that's fine. The learning compounds.
- Don't stop tests early. Peeking inflates false positives.
- Fix the biggest leak first, not the most interesting one.
