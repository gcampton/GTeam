...
|-----------|-----------|------|
| Create context file | `tasks/create.md` | No `~/.agents/product-context.md` yet, or first time on this product |
| Update context file | `tasks/update.md` | File exists but is stale, or a specialist logged update suggestions |

**Routing rules:**
1. Always start by checking: `[ -f ~/.agents/product-context.md ] && echo "EXISTS" || echo "MISSING"`.
2. If MISSING → `tasks/create.md`.
3. If EXISTS and the user wants a refresh, a section edit, or to incorporate signal-triggered updates → `tasks/update.md`.

**Load task:** Read the task file, then execute its workflow.

## Reference Materials

Check `~/dev/1_myprojects/gteam/specialists/product-context/results/` — if result entries exist, Grep them for patterns from prior context-file engagements.

## Notes

- This file is the upstream dependency for every other marketing specialist. Accuracy beats speed.
- Customer Language is the most valuable section — push for verbatim quotes, not paraphrased summaries.
- A positioning claim that could apply to every competitor in the category is not a positioning claim.
