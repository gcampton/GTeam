...
2. **Get the exact download filename** — some configs use `_unified` suffix:
   ...
   ```
   Look for the `file` or config filename in the response.
...
# Get details on a specific config (includes download filename)
curl -s "https://api.skillseekersweb.com/api/configs/<name>"
```
...
**Deliver:** A filtered table of config names → category → description, with the recommended ones highlighted.
...
Install all outputs once complete:
```bash
for name in <name1> <name2> <name3>; do
  cp output/$name/SKILL.md ~/.claude/skills/gteam/specialists/<specialist>/references/$name.md
  echo "Installed: $name.md ($(wc -c < ~/.claude/skills/gteam/specialists/<specialist>/references/$name.md) bytes)"
done
```
...
Re-run the original config or source:
```bash
source ~/dev/skill-seekers/Skill_Seekers/.venv/bin/activate
cd ~/dev/skill-seekers/Skill_Seekers
skill-seekers create configs/gteam/<name>.json -p standard --enhance-level 0
...
Compare size before overwriting:
```bash
echo "Old: $(wc -c < ~/.claude/skills/gteam/specialists/<specialist>/references/<name>.md) bytes"
echo "New: $(wc -c < output/<name>/SKILL.md) bytes"
```

Only overwrite if the new file is larger or the user confirms.
...
- Always use `-p standard --enhance-level 0` as the default. Enhancement can be added later with `skill-seekers enhance`.
...
- After installing a reference file, remind the user that the specialist will load it automatically on the next invocation — no restart needed.
